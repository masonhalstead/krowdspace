...
class ConnectedNavigation extends Component {
    ...
    createUser = () => {
        return (
            <React.Fragment>
                <p onClick={this.handleUserLogin} className="navigation-item">Sign In</p>
                <Button onClick={this.handleCreateAccount} variant="primary" style={{fontSize: '13px', margin: '0px 15px'}}>Create Account</Button>
            </React.Fragment>
        )
    }
    activeUser = () => {
  ...
  render() {
    const { padding, navigation, active_menu, user, display, handleMenuClick } = this.props;
    const style = {
        logo_block: {
            padding: `${padding}px`
        },
    ...
    return (
        <div className="navigation-container">
            <div className="flex-wrapper">
                ...
                <Link to="/">
                <img className="logo-image" src={krowdspace} alt="Krowdspace Logo" height={display.navbar_height-(padding*2)}/>
                </Link>
                </div>
                ...
                <div className="action-wrapper" style={active} onClick={handleMenuClick}>
                    <FontAwesomeIcon
                    icon={['fas', 'bars']}
                    className="action-icon"
                    size="lg"
