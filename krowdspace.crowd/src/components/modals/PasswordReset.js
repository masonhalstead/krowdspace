...
class ConnectedPasswordReset extends Component {
  ...
  isValidForm = () => {
    ...
    if (password && password.length >= 8 && password_strength >= 2) {
  ...
  handleSubmit = e => {
    ...
    api
      .putData('/api/users/password-reset', data, user.token)
      .then(res => {
    ...
    return (
      ...
      >
        ...
        <div className="user-error-body" style={{ textAlign: 'left' }}>
          <p>
            Thank you for creating an account through Google! Please set a new
            password so you can optionally login through Krowdspace. This will
            not prevent you from signing in with your Google account.
          </p>
